...
function nextTypingChunk({
  ...
  const chunk = Math.round(profile.minChunk + (profile.maxChunk - profile.minChunk) * blended);
...
function nextTypingDelay({
  ...
  let delay = profile.baseDelayMs + Math.round(profile.jitterMs * deterministicNoise(cursorEnd + chunkText.length));
...
function scheduleTypingAnimation({
  ...
  const tick = () => {
    const chunkSize = nextTypingChunk({ targetText, cursor, profile });
    ...
    const delay = nextTypingDelay({ targetText, cursorStart, cursorEnd: cursor, profile });
