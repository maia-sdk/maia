...
const EVT_AGENT_WAITING = "agent.waiting";
const EVT_AGENT_HANDOFF = "agent.handoff";
const EVT_AGENT_RESUME = "agent.resume";
const EVT_AGENT_BLOCKED = "agent.blocked";
...
function inferDefaultOverlayForAction({
  ...
  if (normalized === "click") {
    return { text: label ? `Clicking ${label}` : `Clicking ${target}`, variant: "center-pill", pulse: true };
  }
  ...
  if (normalized === "hover") {
    return { text: label ? `Hovering ${label}` : `Hovering ${target}`, variant: "center-pill" };
  }
  if (normalized === "scroll") {
    const direction = normalize(scrollDirection) === "up" ? "up" : "down";
    return { text: `Scrolling ${direction}`, variant: "center-pill", pulse: true };
  }
  if (normalized === "type") {
    return {
...
function overlayForInteractionEvent({
  ...
  if (type === EVT_AGENT_HANDOFF) {
    return { text: "Handing off to next specialist", variant: "left-chip", pulse: true };
  }
  if (type === EVT_AGENT_RESUME) {
    return { text: "Execution resumed", variant: "left-chip", pulse: true };
  }
  if (type === EVT_HANDOFF_RESUMED || type === EVT_APPROVAL_GRANTED) {
    return { text: "Resumed after verification", variant: "left-chip", pulse: true };
  }
  ...
  if (type.startsWith("role_")) {
    return { text: "Switching active role", variant: "left-chip", pulse: true };
  }
  return null;
}
