...
export interface AgentHandoffRelayProps {
  event: RelayEventLike | null;
}
...
const EVT_AGENT_DIALOGUE_STARTED = "agent_dialogue_started";
const EVT_AGENT_DIALOGUE_RESOLVED = "agent_dialogue_resolved";

function toRecord(value: unknown): Record<string, unknown> {
  return value && typeof value === "object" ? (value as Record<string, unknown>) : {};
}

function readEventPayload(input: RelayEventLike | null | undefined): Record<string, unknown> {
  const data = toRecord(input?.data);
  const metadata = toRecord(input?.metadata);
  const nestedData = toRecord(data.data);
  const nestedMetadata = toRecord(metadata.data);
  return {
    ...nestedMetadata,
    ...metadata,
    ...nestedData,
...
function readRelay(event: RelayEventLike | null): RelayPayload | null {
  if (!event) {
    return null;
  }
